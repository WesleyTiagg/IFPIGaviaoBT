import type { Categoria, Produto } from "../entities/menu";
...
function aguardarConsulta(): Promise<void> {
...
export async function listarProdutosPorCategoria(categoriaId: string): Promise<Produto[]> {
  await aguardarConsulta();
  return produtos.filter((produto) => produto.categoriaId === categoriaId);
}

export async function buscarProdutoPorId(produtoId: string): Promise<Produto | undefined> {
  await aguardarConsulta();
